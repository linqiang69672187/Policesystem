...
namespace Policesystem.Handle
{
    ...
    public class getAlarmuser : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            {
                case "331000000000":
                    sqltext.Append("SELECT top 50 gp.ID,al.XM,de.DevId,gp.QQSJ,REPLACE(en.BMMC,'台州市交通警察支队','') BMMC FROM [dbo].[Gps] gp LEFT JOIN Device de on gp.PDAID=de.DevId LEFT JOIN ACL_USER al on de.JYBH = al.JYBH LEFT JOIN Entity en on en.BMDM =de.BMDM  where IsAlarm='1' and al.XM <>'' and gp.QQSJ < GETDATE()-" + alarmdays + " and gp.ID > " + alarmindex + "  ORDER BY gp.ID");
                    break;
                case "331001000000":
                case "331002000000":
                case "331003000000":
                case "331004000000":
                    sqltext.Append("WITH childtable(BMMC,BMDM,SJBM) as (SELECT BMMC,BMDM,SJBM FROM [Entity] WHERE SJBM ='" + BMDM + "' OR BMDM ='" + BMDM + "' UNION ALL SELECT A.BMMC,A.BMDM,A.SJBM FROM [Entity] A,childtable b where a.SJBM = b.BMDM ) SELECT top 50 gp.ID,al.XM,de.DevId,gp.QQSJ,replace(en.BMMC,'台州市交通警察支队','') BMMC FROM [dbo].[Gps] gp LEFT JOIN Device de on gp.PDAID=de.DevId LEFT JOIN ACL_USER al on de.JYBH = al.JYBH  LEFT JOIN Entity en on en.BMDM =de.BMDM where IsAlarm='1'  and  al.BMDM in (SELECT BMDM FROM childtable) and al.XM <>'' and gp.QQSJ < GETDATE()-" + alarmdays + " and gp.ID > " + alarmindex + "    ORDER BY gp.ID");
                    break;
                default:
                    sqltext.Append("SELECT top 50 gp.ID,al.XM,de.DevId,gp.QQSJ,REPLACE(en.BMMC,'台州市交通警察支队','') BMMC FROM [dbo].[Gps] gp LEFT JOIN Device de on gp.PDAID=de.DevId LEFT JOIN ACL_USER al on de.JYBH = al.JYBH  LEFT JOIN Entity en on en.BMDM =de.BMDM where IsAlarm='1'  and al.BMDM='" + BMDM+ "'  and al.XM <>''and gp.QQSJ < GETDATE()-" + alarmdays + "  and gp.ID > " + alarmindex + "  ORDER BY gp.ID");
                    break;
            }
